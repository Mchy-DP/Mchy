...
def test_scope_basic_functions_work():
    ...
    var_scope.register_new_var("var1",  InertType("int"), False, marker, ComLoc())
...
def test_re_definition_fails():
    var_scope = VarScope()
    var_scope.register_new_var("var1",  InertType("int"), False, MarkerDeclVar().with_enclosing_function(None), ComLoc())
    with pytest.raises(ConversionError):
        var_scope.register_new_var("var1",  InertType("str", const=True), True, MarkerDeclVar().with_enclosing_function(None), ComLoc())
